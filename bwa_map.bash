...
#SBATCH --output="$SAVEPATH"bwa_map_out.%j
#SBATCH --error="$SAVEPATH"bwa_map_err.%j
...
echo "Map reads for samples in $SAMPLES FASTQ files in $FASTQ and save at $SAVEPATH. Use $GENOME reference genome and $WINDOWS."
...
echo "$BWA" mem -M -t 16 "$GENOME" "$FASTQ""$SAMPLE".fastq.gz | "$SAMTOOLS" sort -@ 15 -o "$SAVEPATH""$SAMPLE".bwa.sorted.bam -
echo "$SAMPLE".bwa.sorted.bam saved
echo

#index the bam file, print mapping stats to <$SAMPLE>.mappingStats.txt file
echo "$SAMTOOLS" index "$SAVEPATH""$SAMPLE".bwa.sorted.bam
touch "SAVEPATH""$SAMPLE".mappingStats.txt
echo "$SAMTOOLS" flagstat "$SAVEPATH""$SAMPLE".bwa.sorted.bam > "$SAVEPATH""$SAMPLE".mappingStats.txt
...
echo "$SAMTOOLS" view -h -@ 15 -F 3844 -q 1 -M -L "$INCLUDE" "$SAVEPATH""$SAMPLE".bwa.sorted.bam | grep -v -E "SA:Z:|XA:Z:" | "$SAMTOOLS" view -@ 15 -b - | "$SAMTOOLS" markdup -r -f "$SAVEPATH""$SAMPLE".markdups.txt - - | "$BEDTOOLS" genomecov -5 -d -ibam stdin | awk 'BEGIN {OFS="\t"} {if ($3>0) print $1,$2,$2,"name",$3}' > "$SAVEPATH""$SAMPLE".coverage.bed
